...
const loadPostDetail = async () => {
  try {
    const response = await api.get(`/user/post/detail/${route.params.id}`)
...
const uploadImages = async () => {
  ...
  images.value.forEach(image => {
    if (image.file.size > 0) { // 只上传新添加的图片
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 更新帖子
    const response = await api.post('/user/post/update', {
      ...formData.value,
      images: [...imageUrls, ...images.value.filter(img => img.file.size === 0).map(img => img.url)],
      attachments: [...attachmentIds, ...attachments.value.filter(file => file.size === 0).map(file => file.id)]
    })
