                  ...
                  <div class="price-inputs row q-col-gutter-md">
                    ...
                    <div class="col-12 col-md-4">
                      <q-input
                        ...
                        :rules="[
                          val => val >= 0 || '售价不能为负数',
                          val => !dataForm.oriPrice || Number(val) <= Number(dataForm.oriPrice) || '售价不能高于原价'
                        ]"
                        mask="#.##"
                        fill-mask="0"
                        reverse-fill-mask
...
import SkuTag from '~/components/product/SkuEditTag.vue'
import SkuTable from '~/components/product/SkuEditTable.vue'
...
const getDataList = async () => {
  try {
    ...
    if (dataForm.value.prodId) {
      ...
      // 正确处理 SKU 属性数据
      if (data.propList && Array.isArray(data.propList)) {
        skuProps.value = data.propList
      ...
      if (skuTagRef.value?.init) {
        //console.log("初始化 SKU 标签组件，传入数据:", data.skuList)
        skuTagRef.value.init(data.skuList)
      }

      if (skuTableRef.value?.init) {
        //console.log("初始化 SKU 表格组件")
...
const submitForm = async () => {
  ...
  try {
    ...
    //console.log(`准备${method}请求:`, `${API_CONSTANTS.BASE_URL}/prod/prod`)
...
const handleProductTypeChange = (type: ProductType) => {
  ...
  if (type === ProductType.NORMAL) {
    ...
    dataForm.value.presellStatus = 1
    dataForm.value.groupPrice = 0
    dataForm.value.groupMinNum = 2
    dataForm.value.seckillStartTime = ''
    dataForm.value.seckillEndTime = ''
