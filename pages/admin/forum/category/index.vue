        ...
        <template v-slot:default-header="prop">
          <div class="row items-center full-width">
            <q-avatar size="40px" class="q-mr-sm">
              <q-img
                  v-if="prop.node.icon"
                  :src="getImageUrl(prop.node.icon)"
...
// 分类树数据
const categoryTree = ref<ForumCategory[]>([])
// 删除确认对话框
const deleteConfirmDialog = ref(false)
// 添加/编辑分类对话框
const categoryDialog = ref(false)
// 当前选中要删除的分类
const currentDeleteCategory = ref<ForumCategory | null>(null)
// 当前编辑的分类
const currentCategory = ref<ForumCategory | null>(null)
// 是否为编辑模式
const isEditMode = ref(false)
// 分类表单
const categoryForm = ref<ForumCategory>({
  id: null,
  name:"",
  description:"",
  translations: {
...
// 构建树形结构
const buildTree = (flatData: ForumCategory[]): ForumCategory[] => {
  const tree: ForumCategory[] = []
  const map: { [key: string]: ForumCategory } = {}
  ...
  flatData.forEach(item => {
    ...
    if (Number(item.parentId) === 0) {
      ...
      if (parent) {
        ...
        console.warn(`未找到父节点: ${item.parentId}，节点: ${item.id}`)
      }
    }
  })
...
// 打开添加/编辑分类对话框
const openCategoryDialog = (parent: ForumCategory | null = null, editMode: boolean = false) => {
  ...
  if (editMode && parent) {
    ...
    picPreview.value = parent.icon || null
  } else {
    //console.log("添加子类")
    //console.log(parent)
    // 添加子类模式
...
const uploadImage = async (file: File) => {
  ...
  try {
    ...
    if (res.data.code === 200) {
      // 更新表单中的图片URL
      categoryForm.value.icon = res.data.data.sourceUrl
      // 更新预览图
...
const saveCategory = async () => {
  try {
    ...
    const data = {
      ...
      id: isEditMode.value ? categoryForm.value.id : undefined
    }
...
// 删除分类
const handleDelete = (node: ForumCategory) => {
...
const getCurrentLanguageName = (category: ForumCategory) => {
  if (!category.translations) return category.name // 兼容旧数据
